...
class Members(models.Model):
    tag = models.CharField(max_length=12, primary_key=True)
    clanTag = models.ForeignKey(
        Clans, on_delete=models.CASCADE, null=True, related_name="clanTag"
    )
...
class BattleLogs(models.Model):
    ...
    winner2 = models.ForeignKey(
        Members, on_delete=models.CASCADE, null=False, related_name="winner22member"
    )
    loser1 = models.ForeignKey(
        Members, on_delete=models.CASCADE, null=False, related_name="loser12member"
    )
    loser2 = models.ForeignKey(
        Members, on_delete=models.CASCADE, null=False, related_name="loser22member"
    )
